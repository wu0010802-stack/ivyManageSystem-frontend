...
5. 首屏 bundle 維持在預算內（`parent` entry 預算 245 KB gz）。

> **實測結果（2026-09-02 收尾補記）**：底座 237.8 KB → 完成後 238.8 KB，**淨增 1.0 KB**，未達成原本寫的「只減不增」。拆開看：CSS 減 3.5 KB（兩張橫幅與 bento 的樣式退場），JS 增 2.3 KB（待辦清單引入 `M3List`／`M3ListItem`，這兩個元件原本只在 lazy 的事務頁鏈上，現在被拉進首屏）。
>
> 選擇接受這 1 KB 而非改用原生 `ul`／`li`：待辦清單與事務頁呈現同一批資料，列表視覺必須一致；自建第二套列表樣式會偏離設計系統，也要重寫 a11y 行為，代價高於 1 KB。預算餘裕仍有 6.2 KB。
