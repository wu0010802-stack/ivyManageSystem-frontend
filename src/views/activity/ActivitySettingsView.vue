  ...
  <div class="activity-settings">
    ...
    <el-card style="max-width: 720px" v-loading="loading">
      <el-form :model="form" label-width="120px">
        ...
        <el-divider content-position="left">Email 通知模板</el-divider>

        <section class="template-group">
          <h3 class="template-group__title">報名成功通知信</h3>
          <p class="template-hint">
            可用變數：
            <code v-for="token in REGISTRATION_EMAIL_PLACEHOLDERS" :key="token">{{ token }}</code>
          </p>
          <el-form-item label="信件主旨">
            <el-input
              v-model="form.registration_success_email_subject"
              maxlength="200"
              show-word-limit
              placeholder="留空時使用系統預設主旨"
            />
          </el-form-item>
          <el-form-item label="信件內文">
            <el-input
              v-model="form.registration_success_email_body"
              type="textarea"
              :rows="7"
              maxlength="10000"
              show-word-limit
              placeholder="留空時使用系統預設內文；變數由後端寄信時安全替換"
            />
          </el-form-item>
        </section>

        <section class="template-group">
          <h3 class="template-group__title">候補轉正通知信</h3>
          <p class="template-hint">
            可用變數：
            <code v-for="token in WAITLIST_EMAIL_PLACEHOLDERS" :key="token">{{ token }}</code>
          </p>
          <el-form-item label="信件主旨">
            <el-input
              v-model="form.waitlist_promoted_email_subject"
              maxlength="200"
              show-word-limit
              placeholder="留空時使用系統預設主旨"
            />
          ...
          <el-form-item label="信件內文">
            <el-input
              v-model="form.waitlist_promoted_email_body"
              type="textarea"
              :rows="7"
              maxlength="10000"
              show-word-limit
...
const REGISTRATION_EMAIL_PLACEHOLDERS = [
  '{{student_name}}',
  '{{class_name}}',
  '{{school_year}}',
  '{{semester}}',
...
const WAITLIST_EMAIL_PLACEHOLDERS = [
  '{{student_name}}',
  '{{course_name}}',
  '{{confirm_deadline}}',
  '{{query_token}}',
