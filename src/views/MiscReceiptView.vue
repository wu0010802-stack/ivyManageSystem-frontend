<template>
  <div class="misc-receipt-view">
    <!-- 頁首 -->
    <header class="vp-header">
      <div class="vp-header__title">
        <h2>雜項收款簽收</h2>
        <p class="vp-header__sub">逐筆登記雜項收款，上傳繳款方簽收的紙本憑證留存</p>
      </div>
      <el-button
        v-if="canWrite"
        type="primary"
    ...
    <section class="vp-summary" aria-label="本期彙總">
      <div class="vp-summary__cards" v-loading="summaryLoading">
        ...
        <div class="kpi-card" :class="{ 'kpi-warning': summary.pending_count > 0 }">
          <div class="kpi-label">待繳款方簽收</div>
    ...
    >
      <el-table-column prop="receipt_date" label="收款日期" width="116" />
      <el-table-column prop="payer_name" label="繳款方/來源" min-width="160">
        <template #default="{ row }">
          <span class="vp-vendor">{{ row.payer_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="類別" width="104">
        <template #default="{ row }">
          {{ categoryLabel(row.category as string) }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="簽收" min-width="128">
        <template #default="{ row }">
          <template v-if="row.status === 'signed'">
            ...
            <div class="vp-signer__meta">{{ signKindLabel(row.signature_kind as string | null) }}</div>
          </template>
      ...
      <el-table-column label="附件" width="76" align="center">
        <template #default="{ row }">
          <span v-if="(row.attachments as Attachment[] | undefined)?.length" class="vp-attach-chip" title="附件數">
            <el-icon class="vp-attach-chip__clip"><Paperclip /></el-icon>{{ (row.attachments as Attachment[]).length }}
          </span>
      ...
      <template #empty>
        <div class="vp-empty">
          ...
          <template v-else>
            <p class="vp-empty__title">尚無雜項收款紀錄</p>
            <p class="vp-empty__hint">點右上「新增收款」開始登記第一筆雜項收款。</p>
            <el-button v-if="canWrite" type="primary" @click="openCreate">新增收款</el-button>
          </template>
        </div>
      </template>
    </el-table>
    ...
    >
      <el-form :model="form" label-position="top" class="vp-form">
        <div class="vp-form__section">
          <div class="vp-form__section-title">收款資訊</div>
          <div class="vp-form__grid">
            ...
            <el-form-item label="繳款方/來源" required class="vp-form__col-2">
              <el-input v-model="form.payer_name" maxlength="120" :disabled="!canWrite" placeholder="例：台北市教育局" />
            </el-form-item>
            <el-form-item label="類別" required class="vp-form__col-2">
        ...
        <div class="vp-form__section">
          ...
          <el-form-item label="項目／說明">
            <el-input v-model="form.description" maxlength="255" :disabled="!canWrite" placeholder="例：場地租金補助款" />
          </el-form-item>
          <div class="vp-form__grid">
            <el-form-item label="收據／單據號">
              <el-input v-model="form.receipt_number" maxlength="60" :disabled="!canWrite" />
            </el-form-item>
          </div>
        ...
        <div v-if="editingId" class="vp-form__section">
          <div class="vp-form__section-title">
            簽收憑證
            <span class="vp-form__section-hint">翻為「已簽收」所留存的繳款方簽名／簽收照片（1 張）</span>
          </div>
        ...
        <div v-if="editingId" class="vp-form__section">
          <div class="vp-form__section-title">
            單據附件
            <span class="vp-form__section-hint">收據、補助文件等補充資料（最多 5 張）</span>
          </div>
    ...
    <MiscReceiptSignDialog
      v-model="signDialogVisible"
      :receipt-id="signingId"
      @signed="onSigned"
    />
  </div>
</template>
...
const EMPTY_SUMMARY: MiscReceiptSummary = {
  total_count: 0,
  total_amount: 0,
  pending_count: 0,
  pending_amount: 0,
...
const summary = ref<MiscReceiptSummary>({ ...EMPTY_SUMMARY })
...
const dialogTitle = computed(() =>
  editingId.value ? (canWrite.value ? '編輯收款' : '檢視收款') : '新增收款',
)
...
const downloadAttachmentUrl = downloadMiscReceiptAttachmentUrl
const signatureUrl = miscReceiptSignatureUrl
...
function buildRangeParams(): Record<string, unknown> {
  ...
  if (filters.payer_name) params.payer_name = filters.payer_name
...
async function fetchList() {
  ...
  try {
    ...
    const res = await listMiscReceipts(params)
...
async function fetchSummary() {
  ...
  try {
    const res = await getMiscReceiptSummary(buildRangeParams())
    summary.value = res.data as MiscReceiptSummary
  } catch {
...
// 禁未來日：與後端 validate_receipt_date 守衛對齊（收款日不可晚於今日）。
...
async function handleSave() {
  if (!form.receipt_date || !form.payer_name || form.amount == null || !form.category) {
    return ElMessage.warning('請填寫收款日期、繳款方、類別與金額')
  }
  saving.value = true
  try {
    const payload = {
    ...
    if (editingId.value) {
      await updateMiscReceipt(editingId.value, payload)
      ElMessage.success('更新成功')
    } else {
      await createMiscReceipt(payload)
...
async function handleDelete(row: Record<string, unknown>) {
  try {
    await ElMessageBox.confirm(
      `確定刪除「${row.payer_name}」收款紀錄？此動作無法復原。`,
      '確認刪除',
      { type: 'warning', confirmButtonText: '確定刪除', confirmButtonClass: 'el-button--danger' },
    )
    await deleteMiscReceipt(row.id as number)
...
async function handleAttachmentUpload({ file }: { file: File }) {
  ...
  fd.append('file', file)
  try {
    const res = await uploadMiscReceiptAttachment(editingId.value, fd)
...
async function removeAttachment(key: string) {
  try {
    ...
    await deleteMiscReceiptAttachment(editingId.value as number, key)
...
async function tryOpenHighlight() {
  ...
  if (!id) return
  try {
    const res = await getMiscReceipt(id)
    openEdit(res.data)
  } catch {
    ElMessage.warning('找不到該筆收款紀錄')
  }
}
