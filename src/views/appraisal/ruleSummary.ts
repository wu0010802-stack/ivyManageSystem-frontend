...
interface RuleConfig {
  per_unit_delta?: unknown
  unit_cap?: unknown
  delta_cap?: unknown
  per_role_override?: unknown
  input_field?: string
  tiers?: { min?: unknown; delta?: unknown }[]
  threshold?: unknown
  above_delta?: unknown
  below_delta?: unknown
  warning_delta?: unknown
  minor_delta?: unknown
  major_delta?: unknown
  [key: string]: unknown
}
...
const INPUT_FIELD_LABEL: Record<string, string> = {
  retention_rate: '留校率',
  activity_rate: '才藝報名率',
...
function fmtSignedNumber(v: unknown) {
  const n = Number(v)
  if (!Number.isFinite(n)) return String(v)
  return n > 0 ? `+${n}` : String(n)
}

export function summarizeRule(rule: ScoringRule | null | undefined | unknown) {
  ...
  switch (r.rule_type) {
    ...
    default:
      lines.push(`規則類型：${r.rule_type || '未知'}`)
  }
  return lines
}
