...
export function useBodyLock() {
  ...
  function lock() {
    ...
    const cur = window[_LOCK_KEY] || 0
    if (cur === 0) {
      window[_PREV_KEY] = document.body.style.overflow || ''
      document.body.style.overflow = 'hidden'
    }
    window[_LOCK_KEY] = cur + 1
    owns = true
  }
  ...
  function unlock() {
    ...
    window[_LOCK_KEY] = next
    if (next === 0) {
      document.body.style.overflow = window[_PREV_KEY] || ''
      window[_PREV_KEY] = ''
    }
    owns = false
  }
