...
let _govDbCache: Record<string, unknown>[] | null = null   // 已載入的全量資料（陣列）
let _govDbPromise: Promise<Record<string, unknown>[]> | null = null  // in-flight 請求
...
export const loadGovDbData = () => {
  ...
  _govDbPromise = getGovKindergartens({ page_size: 500 })
    .then((res) => {
      const schools = (res.data as { schools?: Record<string, unknown>[] })?.schools ?? []
      _govDbCache = schools
...
const stripGeoType = (name: string) =>
  name
...
// 移除行銷關鍵字（｜或 | 後面的部分）
const stripMarketingTags = (name: unknown) =>
  String(name ?? '').split(/[｜|]/)[0].trim()

// 從 Google Places 名稱萃取多個候選名稱
export const extractCandidates = (name: unknown) => {
...
// 從地址字串萃取行政區（區/鄉/鎮）
const extractDistrict = (address: unknown) => {
  if (!address) return ''
  const m = String(address).match(/([\u4e00-\u9fa5]{1,4}[區鄉鎮市])/)
  return m ? norm(m[1]) : ''
}

// 精確名稱比對
const namesExactMatch = (a: string, b: string) => {
  const na = norm(a)
  const nb = norm(b)
...
// 嚴格名稱比對：包含關係
const namesStrongMatch = (a: string, b: string) => {
...
// 寬鬆名稱比對：3 字元子字串
const namesWeakMatch = (a: string, b: string) => {
...
const _matchFromDb = (schools: Record<string, unknown>[], candidates: string[], hintDistrict: string) => {
  const pools = hintDistrict
    ? [schools.filter((s) => (String(s.district ?? '')).includes(hintDistrict)), schools]
    : [schools]
  for (const matchFn of [namesExactMatch, namesStrongMatch, namesWeakMatch]) {
    for (const pool of pools) {
      for (const candidate of candidates) {
        const found = pool.find((s) => matchFn(String(s.school_name ?? ''), candidate))
...
const _dbSchoolToResult = (found: Record<string, unknown>) => ({
...
export const findPreschoolFromDb = async (searchName: unknown, addressHint = '') => {
...
export const findPreschoolFromDbByNames = async (candidates: string[], addressHint = '') => {
  const schools = await loadGovDbData()
  if (!schools.length) return null
  const hintDistrict = extractDistrict(addressHint)
