...
export const createEmptyAreaHotspotsSummary = (): {
  records_with_address: number; total_hotspots: number; geocoded_hotspots: number; pending_hotspots: number;
  remaining_hotspots: number; failed_hotspots: number; stale_hotspots: number; provider_available: boolean;
  provider_name: null; hotspots: unknown[]
} => ({
...
export const normalizeAreaHotspotsSummary = (payload: Record<string, unknown> = {}) => ({
  ...createEmptyAreaHotspotsSummary(),
  ...payload,
  hotspots: (payload.hotspots as unknown[]) ?? [],
})

export const createEmptyCampus = (fallbackLat: number | null = null, fallbackLng: number | null = null) => ({
  campus_name: '本園',
  campus_address: '',
...
export const createEmptyNearbySchoolState = (): {
  provider_available: boolean; provider_name: string; query_bounds: null; total: number;
  schools: { distance_km?: number; name?: string }[]; message: string
} => ({
  provider_available: false,
...
export const normalizeNearbySchoolState = (payload: Record<string, unknown> = {}) => ({
  ...createEmptyNearbySchoolState(),
  ...payload,
  schools: [...((payload.schools as { distance_km?: number; name?: string }[] | undefined) ?? [])]
    .sort((a, b) => {
...
const roundViewportValue = (value: unknown) => Number(value).toFixed(4)
export const buildNearbyViewportSignature = (bounds: { south?: unknown; west?: unknown; north?: unknown; east?: unknown; zoom?: unknown } = {}) => {
  const parts = [
    bounds.south,
    bounds.west,
    bounds.north,
...
export function useRecruitmentArea({
  ...
  syncBatchSize = 20,
  maxSyncRounds = 100,
  fallbackCampusLat = null,
  fallbackCampusLng = null,
}: {
  ...
  fallbackCampusLat?: number | null;
  fallbackCampusLng?: number | null;
} = {}) {
  ...
  const nearbySchools = ref<{ distance_km?: number; name?: string }[]>([])
  ...
  const buildSyncMessage = (mode: string, payload: Record<string, unknown> = {}) => {
    const attempted = (payload.attempted as number) ?? 0
    const synced = (payload.synced as number) ?? 0
    const failed = (payload.failed as number) ?? 0
    const skipped = (payload.skipped as number) ?? 0
    const remaining = (payload.remaining_hotspots as number) ?? (payload.pending_hotspots as number) ?? 0
    const stale = (payload.stale_hotspots as number) ?? 0
  ...
  const hasRemainingHotspots = (mode: string, payload: Record<string, unknown> = {}) => {
    if (mode === 'resync_google') return ((payload.stale_hotspots as number) ?? 0) > 0
    return ((payload.remaining_hotspots as number) ?? (payload.pending_hotspots as number) ?? 0) > 0
  }

  const buildSyncStateKey = (payload: Record<string, unknown> = {}) => [
    (payload.remaining_hotspots as number) ?? (payload.pending_hotspots as number) ?? 0,
    (payload.stale_hotspots as number) ?? 0,
    (payload.failed_hotspots as number) ?? 0,
    (payload.geocoded_hotspots as number) ?? 0,
  ].join(':')
