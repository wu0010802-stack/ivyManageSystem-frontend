...
export function useWorkdayCalculator({ form, fetchFn = null }: { form: Record<string, unknown>; fetchFn?: ((...args: unknown[]) => Promise<unknown>) | null }) {
  const calcHint = ref('')
  const calcBreakdown = ref<{ type: string; date: string; hours?: number; work_start?: string; work_end?: string }[]>([])
  const calcLoading = ref(false)
  const leaveMode = ref('full')
  const leaveSingleDate = ref('')

  const countWorkdays = (startDate: Date | string, endDate: Date | string) => {
    let count = 0
  ...
  const calcSameDayHours = (start: Date, end: Date) => {
  ...
  const fallbackCalc = (start: string, end: string) => {
  ...
  const applyBreakdownResult = (start: string, end: string, total_hours: number, breakdown: { type: string; date: string; hours?: number; work_start?: string; work_end?: string }[]) => {
    ...
    if (leaveMode.value === 'morning') {
      ...
      const hours = Math.max(0.5, Math.round((new Date(form.end_date as string).getTime() - new Date(form.start_date as string).getTime()) / 3600000 * 2) / 2)
  ...
  const resetCalculatorState = (formRef: { value?: { clearValidate?: () => void } } | null | undefined) => {
    calcHint.value = ''
    calcBreakdown.value = []
    calcLoading.value = false
  ...
  const getExpectedMaxHours = () => {
    const s = new Date(form.start_date as string)
    const e = new Date(form.end_date as string)
    const sDay = new Date(s)
    sDay.setHours(0, 0, 0, 0)
    const eDay = new Date(e)
    eDay.setHours(0, 0, 0, 0)
  ...
  watch(leaveMode, (newMode) => {
    ...
    if (newMode === 'full') {
      if (leaveSingleDate.value) {
        ...
        form.start_date = (form.start_date as string).substring(0, 10)
        form.end_date = form.end_date ? (form.end_date as string).substring(0, 10) : ''
      }
      leaveSingleDate.value = ''
      return
    }
    if (newMode === 'morning' || newMode === 'afternoon') {
      const dateStr = form.start_date ? (form.start_date as string).substring(0, 10) : ''
