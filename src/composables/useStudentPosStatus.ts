 ...
 *
 * 防禦性優先權：同一學生同日理論上只會有一種完成路徑，但若資料異常同時存在
 * proxy 與非 proxy 的 completed call，proxy_picked 優先——委託代理人代接是需要
 * 辦公室特別留意的較窄訊號（人不是家長本人），異常時寧可多顯示這個提醒。
 */
export function useStudentPosStatus(
  student: PosStudentInput,
  calls: PosStudentCallInput[],
): PosStudentStatusResult {
  const status: PosStudentStatus = hasCompletedCall(student.id, calls, 'proxy')
    ? 'proxy_picked'
